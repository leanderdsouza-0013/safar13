...
import OperatorDetails from "./OperatorDetail"; // Import the OperatorDetails component
...
const DashManageOperator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!companyName) { errors.companyName = "Please enter company name"; }
    ...
    try {
      const response = await axios.post("/api/admin/new_busOperator", { name, email, password, companyName, phone});
      ...
      <div className="add-operator-container">
        ...
        <form className="add-operator-form" onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            ...
            <input
              type="text"
              className={`form-control ${error.companyName && 'is-invalid'}`}
              id="companyName"
              value={companyName}
              onChange={(e) => setCompanyName(e.target.value)}
              required
            />
            {error.companyName && <div className="invalid-feedback">{error.companyName}</div>}
          </div>
          {/* Phone Number */}
          <div className="mb-3">
